...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"xdnstats", "explorer", "wallet", "faucet"} {
	...
	// If we have xdnstats running, ask whxdner to make the secret public or not
	var xdnstats bool
	if w.conf.xdnstats != "" {
		fmt.Println()
		fmt.Println("Include xdnstats secret on dashboard (y/n)? (default = yes)")
		xdnstats = w.readDefaultString("y") == "y"
	}
	// Try to deploy the dashboard container on the host
	if out, err := deployDashboard(client, w.network, infos.port, infos.host, listing, &w.conf, xdnstats); err != nil {
