...
func (w *wizard) networkStats(tips bool) {
	...
	for server, pubkey := range w.conf.Servers {
		...
		logger.Debug("Checking for xdnstats availability")
		if infos, err := checkDnpstats(client, w.network); err != nil {
			if err != ErrServiceUnknown {
				services["xdnstats"] = err.Error()
			}
		} else {
			services["xdnstats"] = infos.String()
			protips.xdnstats = infos.config
		}
	...
	if protips.xdnstats != "" {
		w.conf.xdnstats = protips.xdnstats
	}
...
func (p *protips) print(network string) {
	...
	fullinit, lightinit := "", ""
	if p.genesis != "" {
		fullinit = fmt.Sprintf("gxdn --datadir=$HOME/.%s init %s.json && ", network, network)
		lightinit = fmt.Sprintf("gxdn --datadir=$HOME/.%s --light init %s.json && ", network, network)
	}
	// If an xdnstats server is available, add the xdnstats flag
	statsflag := ""
	if p.xdnstats != "" {
		if strings.Contains(p.xdnstats, " ") {
			statsflag = fmt.Sprintf(` --xdnstats="yournode:%s"`, p.xdnstats)
		} else {
			statsflag = fmt.Sprintf(` --xdnstats=yournode:%s`, p.xdnstats)
		}
	}
	...
	tips = append(tips, fmt.Sprintf("%sgxdn --networkid=%d --datadir=$HOME/.%s --cache=1024%s%s", fullinit, p.network, network, statsflag, bootflagFull))

	tasks = append(tasks, "Run a full node with recent data only")
	tips = append(tips, fmt.Sprintf("%sgxdn --networkid=%d --datadir=$HOME/.%s --cache=512 --fast%s%s", fullinit, p.network, network, statsflag, bootflagFull))

	tasks = append(tasks, "Run a light node with on demand retrievals")
	tips = append(tips, fmt.Sprintf("%sgxdn --networkid=%d --datadir=$HOME/.%s --light%s%s", lightinit, p.network, network, statsflag, bootflagLight))

	tasks = append(tasks, "Run an embedded node with constrained memory")
	tips = append(tips, fmt.Sprintf("%sgxdn --networkid=%d --datadir=$HOME/.%s --cache=32 --light%s%s", lightinit, p.network, network, statsflag, bootflagLight))
