...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many Dnpers to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.xdnstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.xdnstats = w.readString() + ":" + w.conf.xdnstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.xdnstats)
		infos.node.xdnstats = w.readDefaultString(infos.node.xdnstats) + ":" + w.conf.xdnstats
	}
