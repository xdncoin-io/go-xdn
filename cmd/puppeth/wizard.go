...
// config contains all the configurations needed by puppxdn that should be saved
// between sessions.
type config struct {
	...
	xdnstats  string        // Dnpstats settings to cache for node deploys
...
func (c config) flush() {
	...
	if err := ioutil.WriteFile(c.path, out, 0644); err != nil {
		log.Warn("Failed to save puppxdn configs", "file", c.path, "err", err)
	}
}
...
type wizard struct {
	...
	services map[string][]string   // Dnp services known to be running on servers
...
// converts it to an Dnp address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by xdnstats, which compares IPs textually, not by value.
