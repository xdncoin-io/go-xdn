...
// makeWizard creates and returns a new puppxdn wizard.
...
// setting up a new or managing an existing Dnp private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to puppxdn, your Dnp private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new Dnp network down to  |")
	fmt.Println("| the genesis block, bootnodes, miners and xdnstats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| Puppxdn uses SSH to dial in to remote servers, and builds |")
	...
	log.Info("Administering Dnp network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".puppxdn", w.network)
	...
	for {
		...
		default:
			log.Error("That's not somxdning I can do")
		}
	}
}
