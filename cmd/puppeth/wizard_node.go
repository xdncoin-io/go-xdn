...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.xdnstats == "" {
		log.Error("No xdnstats server configured")
		return
	}
	...
	if infos.xdnstats == "" {
		...
		infos.xdnstats = w.readString() + ":" + w.conf.xdnstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.xdnstats)
		infos.xdnstats = w.readDefaultString(infos.xdnstats) + ":" + w.conf.xdnstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.genesis.Config.Dnpash != nil {
			// Dnpash based miners only need an xdnerbase to mine against
			fmt.Println()
			if infos.xdnerbase == "" {
				fmt.Printf("What address should the miner user?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.xdnerbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner user? (default = %s)\n", infos.xdnerbase)
				infos.xdnerbase = w.readDefaultAddress(common.HexToAddress(infos.xdnerbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootFull, w.conf.bootLight, infos); err != nil {
		log.Error("Failed to deploy Dnp node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
