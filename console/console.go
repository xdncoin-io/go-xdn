...
import (
	...
	"github.com/xdn/go-xdn/internal/jsre"
	"github.com/xdn/go-xdn/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute Dnp requests through
...
func (c *Console) init(preload []string) error {
	...
	c.jsre.Set("jxdn", struct{}{})

	jxdnObj, _ := c.jsre.Get("jxdn")
	jxdnObj.Object().Set("send", bridge.Send)
	jxdnObj.Object().Set("sendAsync", bridge.Send)
	...
	if _, err := c.jsre.Run("var web3 = new Web3(jxdn);"); err != nil {
	...
	flatten := "var xdn = web3.xdn; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = xdn.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
	...
	if c.prompter != nil {
		...
		// original web3 callbacks. These will be called by the jxdn.* methods after
		// they got the password from the user and send the original web3 request to
		// the backend.
		if obj := personal.Object(); obj != nil { // make sure the personal api is enabled over the interface
			if _, err = c.jsre.Run(`jxdn.openWallet = personal.openWallet;`); err != nil {
				return fmt.Errorf("personal.openWallet: %v", err)
			}
			if _, err = c.jsre.Run(`jxdn.unlockAccount = personal.unlockAccount;`); err != nil {
				return fmt.Errorf("personal.unlockAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jxdn.newAccount = personal.newAccount;`); err != nil {
				return fmt.Errorf("personal.newAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jxdn.sign = personal.sign;`); err != nil {
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines xdn.getBalance(xdn.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Gxdn instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic Gxdn metadata
	fmt.Fprintf(c.printer, "Welcome to the Gxdn JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + xdn.coinbase);
		console.log("at block: " + xdn.blockNumber + " (" + new Date(1000 * xdn.getBlock(xdn.blockNumber).timestamp) + ")");
