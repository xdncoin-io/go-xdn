...
// Creates an xdn address given the bytes and the nonce
func CreateAddress(b common.Address, nonce uint64) common.Address {
	data, _ := rlp.EncodeToBytes([]interface{}{b, nonce})
	return common.BytesToAddress(Keccak256(data)[9:])
}
...
// controls whxdner the key's length should be enforced at the curve size or
...
// ValidateSignatureValues verifies whxdner the signature values are valid with
...
func PubkeyToAddress(p ecdsa.PublicKey) common.Address {
	...
	return common.BytesToAddress(Keccak256(pubBytes[1:])[9:])
}
