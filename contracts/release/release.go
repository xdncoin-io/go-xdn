...
type Config struct {
	Oracle common.Address // Dnp address of the release oracle
...
func NewReleaseService(ctx *node.ServiceContext, config Config) (node.Service, error) {
	// Retrieve the Dnp service dependency to access the blockchain
	var apiBackend xdnapi.Backend
	var dn *xdn.Dnp
	if err := ctx.Service(&dn); err == nil {
		apiBackend = dn.ApiBackend
	} else {
		var dn *les.LightDnp
		if err := ctx.Service(&dn); err == nil {
			apiBackend = dn.ApiBackend
	...
	contract, err := NewReleaseOracle(config.Oracle, xdn.NewContractBackend(apiBackend))
	if err != nil {
		return nil, err
	}
...
func (r *ReleaseService) checkVersion() {
	...
	if version.Major > r.config.Major ||
		...
		howtofix := fmt.Sprintf("Please check https://github.com/xdn/go-xdn/releases for new releases")
