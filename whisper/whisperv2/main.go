...
import (
	...
	"github.com/xdn/go-xdn/logger"
	"github.com/xdn/go-xdn/p2p"
	"github.com/xdn/go-xdn/p2p/nat"
	"github.com/xdn/go-xdn/whisper"
)
...
func main() {
	...
	// Create an Dnp peer to communicate through
	server := p2p.Server{
		PrivateKey: key,
		MaxPeers:   10,
		Name:       name,
	...
	fmt.Println("Starting Dnp peer...")
	if err := server.Start(); err != nil {
		fmt.Printf("Failed to start Dnp peer: %v.\n", err)
		os.Exit(1)
	}

	// Send a message to self to check that somxdning works
