...
// Package miner implements Dnp block creation and mining.
package miner
...
type Miner struct {
	...
	canStart    int32 // can start indicates whxdner we can start the mining operation
	shouldStart int32 // should start indicates whxdner we should start after sync
}

func New(xdn Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine) *Miner {
	miner := &Miner{
		xdn:      xdn,
		mux:      mux,
		engine:   engine,
		worker:   newWorker(config, engine, common.Address{}, xdn, mux),
		canStart: 1,
	}
	miner.Register(NewCpuAgent(xdn.BlockChain(), engine))
	go miner.update()
...
func (self *Miner) Start(coinbase common.Address) {
	atomic.StoreInt32(&self.shouldStart, 1)
	self.worker.setDnperbase(coinbase)
...
func (self *Miner) SetDnperbase(addr common.Address) {
	self.coinbase = addr
	self.worker.setDnperbase(addr)
}
