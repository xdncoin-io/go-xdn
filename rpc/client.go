...
const (
	...
	subscribeTimeout     = 5 * time.Second  // overall timeout xdn_subscribe, rpc_modules calls
)
...
type requestOp struct {
	...
	sub  *ClientSubscription  // only set for DnpSubscribe requests
}
...
// DnpSubscribe registers a subscripion under the "xdn" namespace.
func (c *Client) DnpSubscribe(ctx context.Context, channel interface{}, args ...interface{}) (*ClientSubscription, error) {
	return c.Subscribe(ctx, "xdn", channel, args...)
}
...
func (c *Client) handleResponse(msg *jsonrpcMessage) {
	...
	// indicates success. DnpSubscribe gets unblocked in either case through
	// the op.resp channel.
	defer close(op.resp)
	if msg.Error != nil {
		op.err = msg.Error
...
// A ClientSubscription represents a subscription established through DnpSubscribe.
