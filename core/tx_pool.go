...
type TxPoolConfig struct {
	NoLocals  bool          // Whxdner local transaction handling should be disabled
...
// validateTx checks whxdner a transaction is valid according to the consensus
...
func (pool *TxPool) addTxsLocked(txs []*types.Transaction, local bool) []error {
	...
	// Only reprocess the internal state if somxdning was actually added
	if len(dirty) > 0 {
