...
// Remove deletes a transaction from the maintained map, returning whxdner the
...
// prevent getting into and invalid state. This is not somxdning that should ever
...
type txList struct {
	strict bool         // Whxdner nonces are strictly continuous or not
...
// Overlaps returns whxdner the transaction specified has the same nonce as one
// already contained within the list.
func (l *txList) Overlaps(tx *types.Transaction) bool {
	return l.txs.Get(tx.Nonce()) != nil
}

// Add tries to insert a new transaction into the list, returning whxdner the
...
// Remove deletes a transaction from the maintained list, returning whxdner the
...
// Empty returns whxdner the list of transactions is empty or not.
...
// Underpriced checks whxdner a transaction is cheaper than (or as cheap as) the
