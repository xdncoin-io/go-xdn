...
func GenerateChain(config *params.ChainConfig, parent *types.Block, db xdndb.Database, n int, gen func(int, *BlockGen)) ([]*types.Block, []types.Receipts) {
	if config == nil {
...
// header only chain.
func newCanonical(n int, full bool) (xdndb.Database, *BlockChain, error) {
	// Initialize a fresh chain with only a genesis block
	gspec := new(Genesis)
	db, _ := xdndb.NewMemDatabase()
	genesis := gspec.MustCommit(db)

	blockchain, _ := NewBlockChain(db, params.AllDnpashProtocolChanges, ethash.NewFaker(), vm.Config{})
...
func makeHeaderChain(parent *types.Header, n int, db xdndb.Database, seed int) []*types.Header {
...
func makeBlockChain(parent *types.Block, n int, db xdndb.Database, seed int) []*types.Block {
