...
var (
	...
	configPrefix   = []byte("xdn-config-") // config prefix for the db
...
func WriteCanonicalHash(db xdndb.Putter, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db xdndb.Putter, hash common.Hash) error {
...
func WriteHeadBlockHash(db xdndb.Putter, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db xdndb.Putter, hash common.Hash) error {
...
func WriteHeader(db xdndb.Putter, header *types.Header) error {
...
func WriteBody(db xdndb.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
func WriteBodyRLP(db xdndb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
func WriteTd(db xdndb.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteBlock(db xdndb.Putter, block *types.Block) error {
...
func WriteBlockReceipts(db xdndb.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
...
func WriteTxLookupEntries(db xdndb.Putter, block *types.Block) error {
...
func WriteBloomBits(db xdndb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func PreimageTable(db xdndb.Database) xdndb.Database {
	return xdndb.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db xdndb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
func WriteBlockChainVersion(db xdndb.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db xdndb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
