...
// Package core implements the Dnp consensus protocol.
package core
...
// available in the database. It initialises the default Dnp Validator and
// Processor.
func NewBlockChain(chainDb xdndb.Database, config *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
...
func (bc *BlockChain) ResetWithGenesisBlock(genesis *types.Block) error {
	...
	if err := bc.hc.WriteTd(genesis.Hash(), genesis.NumberU64(), new(big.Int)); err != nil {
...
func (bc *BlockChain) GetBlockByNumber(number uint64) *types.Block {
	...
	// cellBytes := poc.GenCell(uint64(12345), uint64(18446744073709551615))
	// cellFile := poc.GetCellFromPlot("/home/ubuntu/ndp/P_DATA", uint64(12345), uint64(18446744073709551615))
	// poc.GenCell(uint64(12345), uint64(18446744073709551615))
	// poc.GenCell(uint64(12346), uint64(18446744073709551615))
	// poc.GenCell(uint64(12347), uint64(18446744073709551615))
	// poc.GetCellFromPlot("/home/ubuntu/ndp/P_DATA", uint64(12345), uint64(18446744073709551615))
	// fmt.Printf("celllen:%v, filelen:%v\r\n", len(cellBytes), len(cellFile))
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
	...
	for i, block := range blockChain {
		...
		if batch.ValueSize() >= xdndb.IdealBatchSize {
...
func (bc *BlockChain) insertChain(chain types.Blocks) (int, []interface{}, []*types.Log, error) {
	...
	for i, block := range chain {
		...
		case SideStatTy:
			log.Info("Inserted forked block", "number", block.Number(), "hash", block.Hash(), "elapsed",
...
//
// The verify parameter can be used to fine tune whxdner nonce verification
