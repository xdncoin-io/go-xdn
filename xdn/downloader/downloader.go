...
var (
	...
	maxQueuedHeaders  = 32 * 1024 // [xdn/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	// Channels
	headerCh      chan dataPack        // [xdn/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [xdn/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [xdn/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [xdn/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [xdn/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*types.Header // [xdn/62] Channel to feed the header processor new tasks

	// for stateFetcher
	stateSyncStart chan *stateSync
	trackStateReq  chan *stateReq
	stateCh        chan dataPack // [xdn/63] Channel receiving inbound node state data
...
func New(mode SyncMode, stateDb xdndb.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
...
// these are zero.
func (d *Downloader) Progress() xdn.SyncProgress {
...
// Synchronising returns whxdner the downloader is currently retrieving blocks.
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "xdn", p.version, "head", hash, "td", td, "mode", d.mode)
...
func (d *Downloader) fetchHeight(p *peerConnection) (*types.Header, error) {
	...
	for {
		...
		case packet := <-d.headerCh:
			...
			// Make sure the peer actually gave somxdning valid
	...
	for {
		...
		case packet := <-deliveryCh:
			...
			if peer := d.peers.Peer(packet.PeerId()); peer != nil {
				...
				// Unless a peer delivered somxdning completely else than requested (usually
	...
	for {
		...
		case headers := <-d.headerProcCh:
			...
			if len(headers) == 0 {
				...
				// L: Request new headers up from 11 (R's TD was higher, it must have somxdning)
				// R: Nothing to give
				if d.mode != LightSync {
				...
				// peer gave us somxdning useful, we're already happy/progressed (above check).
			...
			for len(headers) > 0 {
				// Terminate if somxdning failed in between processing chunks
				select {
				case <-d.cancelCh:
					return errCancelHeaderProcessing
				default:
