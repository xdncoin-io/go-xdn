...
var (
	...
	// sharedDnpash is a full instance that can be shared between multiple users.
	sharedDnpash = New("", 3, 0, "", 1, 0)
...
// isLittleEndian returns whxdner the local system is running in little or big
// endian byte order.
...
// Dnpash is a consensus engine based on proot-of-work implementing the ethash
// algorithm.
type Dnpash struct {
	...
	tester    bool          // Flag whxdner to use a smaller test dataset
	shared    *Dnpash       // Shared PoW verifier to avoid cache regeneration
	fakeMode  bool          // Flag whxdner to disable PoW checking
	fakeFull  bool          // Flag whxdner to disable all consensus rules
...
func New(cachedir string, cachesinmem, cachesondisk int, dagdir string, dagsinmem, dagsondisk int) *Dnpash {
	if cachesinmem <= 0 {
...
// all blocks' seal as valid, though they still have to conform to the Dnp
// consensus rules.
func NewFaker() *Dnpash {
	return &Dnpash{fakeMode: true}
}

// NewFakeFailer creates a ethash consensus engine with a fake PoW scheme that
// accepts all blocks as valid apart from the single one specified, though they
// still have to conform to the Dnp consensus rules.
func NewFakeFailer(fail uint64) *Dnpash {
	return &Dnpash{fakeMode: true, fakeFail: fail}
}

// NewFakeDelayer creates a ethash consensus engine with a fake PoW scheme that
// accepts all blocks as valid, but delays verifications by some time, though
// they still have to conform to the Dnp consensus rules.
func NewFakeDelayer(delay time.Duration) *Dnpash {
	return &Dnpash{fakeMode: true, fakeDelay: delay}
}

// NewFullFaker creates an ethash consensus engine with a full fake scheme that
// accepts all blocks as valid, without checking any consensus rules whatsoever.
func NewFullFaker() *Dnpash {
	return &Dnpash{fakeMode: true, fakeFull: true}
}

// NewShared creates a full sized ethash PoW shared between all requesters running
// in the same process.
func NewShared() *Dnpash {
	return &Dnpash{shared: sharedDnpash}
}

// cache tries to retrieve a verification cache for the specified block number
// by first checking against a list of in-memory caches, then against caches
// stored on disk, and finally generating one if none can be found.
func (ethash *Dnpash) cache(block uint64) []uint32 {
...
// stored on disk, and finally generating one if none can be found.
func (ethash *Dnpash) dataset(block uint64) []uint32 {
	epoch := block / epochLength
...
func (ethash *Dnpash) Threads() int {
	ethash.lock.Lock()
...
// work being done.
func (ethash *Dnpash) SetThreads(threads int) {
	ethash.lock.Lock()
	defer ethash.lock.Unlock()
...
func (ethash *Dnpash) Hashrate() float64 {
	return ethash.hashrate.Rate1()
}

// APIs implements consensus.Engine, returning the user facing RPC APIs. Currently
// that is empty.
func (ethash *Dnpash) APIs(chain consensus.ChainReader) []rpc.API {
	return nil
}
