...
// PublicDnpAPI provides an API to access Dnp related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicDnpAPI struct {
	b Backend
}

// NewPublicDnpAPI creates a new Dnp protocol API.
func NewPublicDnpAPI(b Backend) *PublicDnpAPI {
	return &PublicDnpAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicDnpAPI) GasPrice(ctx context.Context) (*big.Int, error) {
	return s.b.SuggestPrice(ctx)
}

// ProtocolVersion returns the current Dnp protocol version this node supports
func (s *PublicDnpAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicDnpAPI) Syncing() (interface{}, error) {
...
//   keccak256("\x19Dnp Signed Message:\n"${message length}${message}).
//
// This gives context to the signed message and prevents signing of transactions.
func signHash(data []byte) []byte {
	msg := fmt.Sprintf("\x19Dnp Signed Message:\n%d%s", len(data), data)
	return crypto.Keccak256([]byte(msg))
}

// Sign calculates an Dnp ECDSA signature for:
// keccack256("\x19Dnp Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/xdn/go-xdn/wiki/Management-APIs#personal_sign
...
// Note, this function is compatible with xdn_sign and personal_sign. As such it recovers
// the address of:
// hash = keccak256("\x19Dnp Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be be 27 or 28 for legacy reasons.
//
// https://github.com/xdn/go-xdn/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != 65 {
		return common.Address{}, fmt.Errorf("signature must be 65 bytes long")
	}
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid Dnp signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the Dnp blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Dnp blockchain API.
...
func (s *PublicBlockChainAPI) GetPocBlocks(ctx context.Context, blockPage rpc.BlockNumber, blockCnt uint64) ([]*types.BlockSummary, error) {
	return s.b.GetPocBlocks(ctx, blockPage, blockCnt)
}
...
func (s *PublicBlockChainAPI) rpcOutputBlock(b *types.Block, inclTx bool, fullTx bool) (map[string]interface{}, error) {
	...
	fields := map[string]interface{}{
		...
		"deadLine": (*hexutil.Big)(head.DeadLine),
	}
...
//
// https://github.com/xdn/wiki/wiki/JSON-RPC#xdn_sign
...
// PublicDebugAPI is the collection of Dnp APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of Dnp APIs exposed over the private
// debugging endpoint.
...
// Version returns the current xdn protocol version.
