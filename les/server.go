...
import (
	...
	"github.com/xdn/go-xdn/les/flowcontrol"
...
func NewLesServer(xdn *xdn.Dnp, config *xdn.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(xdn.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, xdn.EventMux(), xdn.Engine(), newPeerSet(), xdn.BlockChain(), xdn.TxPool(), xdn.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range ServerProtocolVersions {
		lesTopics[i] = lesTopic(xdn.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		...
		quitSync:         quitSync,
		lesTopics:        lesTopics,
		chtIndexer:       light.NewChtIndexer(xdn.ChainDb(), false),
		bloomTrieIndexer: light.NewBloomTrieIndexer(xdn.ChainDb(), false),
	}
	logger := log.New()
	...
	srv.chtIndexer.Start(xdn.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(xdn.ChainDb())
	return srv, nil
}
...
func newCostStats(db xdndb.Database) *requestCostStats {
