...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Dnp
...
type wallet struct {
	...
	deriveChain    xdn.ChainStateReader // Blockchain state reader to discover used account with
...
// whxdner they are still present or if they malfunctioned.
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Dnp account
			...
			// If the next account is empty, stop self-derivation, but add it nonxdneless
...
// Contains implements accounts.Wallet, returning whxdner a particular account is
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain xdn.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
