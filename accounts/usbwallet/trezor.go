...
import (
	...
	"github.com/xdn/go-xdn/accounts/usbwallet/internal/trezor"
...
type trezorDriver struct {
	...
	pinwait bool          // Flags whxdner the device is waiting for PIN entry
...
// Status implements accounts.Wallet, always whxdner the Trezor is opened, closed
// or whxdner the Dnp app was not started on it.
...
// Dnp address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.DnpAddress)
	if _, err := w.trezorExchange(&trezor.DnpGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.DnpTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.DnpTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
