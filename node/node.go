...
func (n *Node) OpenDatabase(name string, cache, handles int) (xdndb.Database, error) {
	if n.config.DataDir == "" {
		return xdndb.NewMemDatabase()
	}
	return xdndb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
