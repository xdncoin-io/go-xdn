...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (xdndb.Database, error) {
	if ctx.config.DataDir == "" {
		return xdndb.NewMemDatabase()
	}
	db, err := xdndb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
