...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gxdn is "gxdn". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gxdn", keep that.
	if name == "gxdn" || name == "gxdn-testnet" {
		name = "Gxdn"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gxdn" instances.
var isOldGxdnResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) resolvePath(path string) string {
	...
	// by gxdn 1.4 are used if they exist.
	if c.name() == "gxdn" && isOldGxdnResource[path] {
		oldpath := ""
		if c.Name == "gxdn" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-xdn-keystore")
		ephemeral = keydir
	}
