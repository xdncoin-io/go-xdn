...
import (
	...
	"github.com/xdn/go-xdn/common/bitutil"
...
func GetChtRoot(db xdndb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
...
func GetChtV2Root(db xdndb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	return GetChtRoot(db, (sectionIdx+1)*(ChtFrequency/ChtV1Frequency)-1, sectionHead)
}

// StoreChtRoot writes the CHT root assoctiated to the given section into the database
// Note that sectionIdx is specified according to LES/1 CHT section size
func StoreChtRoot(db xdndb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func NewChtIndexer(db xdndb.Database, clientMode bool) *core.ChainIndexer {
	cdb := xdndb.NewTable(db, ChtTablePrefix)
	idb := xdndb.NewTable(db, "chtIndex-")
...
const (
	...
	xdnBloomBitsSection       = 4096
	xdnBloomBitsConfirmations = 256
)
...
func GetBloomTrieRoot(db xdndb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db xdndb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
// NewBloomTrieIndexer creates a BloomTrie chain indexer
func NewBloomTrieIndexer(db xdndb.Database, clientMode bool) *core.ChainIndexer {
	cdb := xdndb.NewTable(db, BloomTrieTablePrefix)
	idb := xdndb.NewTable(db, "bltIndex-")
	...
	if clientMode {
		...
		backend.parentSectionSize = xdnBloomBitsSection
		confirmReq = HelperTrieProcessConfirmations
	}
	backend.bloomTrieRatio = BloomTrieFrequency / backend.parentSectionSize
	backend.sectionHeads = make([]common.Hash, backend.bloomTrieRatio)
	return core.NewChainIndexer(db, idb, backend, BloomTrieFrequency, confirmReq-xdnBloomBitsConfirmations, time.Millisecond*100, "bloomtrie")
}
